...
void colourPhase() {//ESTA ES MI FUNCION
  allRgbValueOne();
  ...
  // Calcule la altura de la barra en función de los niveles mínimos/máximos dinámicos (punto fijo):
...
#pragma region commandlTopic
  if (String(commandlTopic).equals(topic)) {
    if (length == 1) {
      ...
      DynamicJsonDocument root(256);
      DeserializationError error = deserializeJson(root, message_buff);
